...
export class SelectsBox extends Phaser.GameObjects.Container {
    ...
    constructor(
        public scene: Phaser.Scene, { x, y, width, height, padding = 10, margin = 10, textStyle = {} }: SelectsBoxConfig) {
        ...
        // 選択テキスト用のTextを作成
        this.text1 =
            new Phaser.GameObjects.Text(this.scene, x - width / 2 + padding, y - height / 2 + padding, '', dialogBoxTextStyle);
        this.text2 =
            new Phaser.GameObjects.Text(this.scene, x - width / 2 + padding, y - height / 2 + 30 + padding, '', dialogBoxTextStyle);
        this.text3 =
            new Phaser.GameObjects.Text(this.scene, x - width / 2 + padding, y - height / 2 + 60 + padding, '', dialogBoxTextStyle);
