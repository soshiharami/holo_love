...
export class MainScene extends Phaser.Scene {
  ...
  create() {
    ...
    var dialoguies: (NomalDialogue | SelectsDialogue)[] =
      story.chapters[0].scenies[0].dialogue;
    ...
    const backgroundImage = this.add.image(
      width / 2,
      height / 2,
      nowScene.backgroundImage,
    );

    const isNomal = (
      nomal: NomalDialogue | SelectsDialogue,
    ): nomal is NomalDialogue => {
      return nomal.kind === "nomal";
    };

    const isSelects = (
      nomal: NomalDialogue | SelectsDialogue,
    ): nomal is SelectsDialogue => {
    ...
    const rendering = (
      nextDialogue: NomalDialogue | SelectsDialogue | undefined,
    ): void => {
      if (nextDialogue) {
        ...
        if (isNomal(nowDialogue)) {
          ...
          nowRigft = this.add.image(
            width - 200,
            height + 100,
            nowDialogue.auther.stylies.normal?.image ?? "",
          );
